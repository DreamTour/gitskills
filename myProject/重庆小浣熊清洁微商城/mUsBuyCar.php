        ...
        <ul class="wrap">
            ...
            <li class="payNow">
                <form name="payForm" method="post" action="<?php echo root."pay/wxpay/wxpay.php";?>">
                    ...
                    <input type="submit" class="confrim" id="bind-phone_btn" value="立即支付">
                </form>
            </li>
        </ul>
    </div>
    ...
    function TotalPrice() {
        ...
        $(".buycart").each(function() { //循环每个店铺
            ...
            $(this).find(".GoodsCheck").each(function() { //循环店铺里面的商品
                ...
                var groupBuy = <?php echo para("groupBuy");?>; //团购（满几件打折）
                var discountMoney = <?php echo para("discountMoney");?>; //折扣价（满多少元打折）
                ...
                if (totalPrice > 0) {
                    ...
                    if (num >= groupBuy || totalPrice >= discountMoney) { //购买数量大于等于团购数量打折或者商品价格大于等于折扣价
                        totalPrice = (parseFloat(totalPrice) * 0.9).toFixed(2);
                        total = parseFloat(totalPrice) + parseFloat(freight);
                        $("#money-mx").html("元（商品优惠价："+ totalPrice +"元(9折) + 免邮费）");
                    }else{
                        total = parseFloat(totalPrice) + parseFloat(freight);
                        $("#money-mx").html("元（商品总价："+ totalPrice +"元 + 邮费："+ freight +"元）");
                    }
                }else{
                    total = totalPrice;  //总价
