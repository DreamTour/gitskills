	...
	if(empty($type)){
		...
		if($adDuty['name'] != "超级管理员"){
			...
			foreach($Array as $id){
				...
				//删除客户预支付记录
				mysql_query("delete from recharge where TargetId = '$id'");
				//删除客户发布盘州信息或者新感悟
				mysql_query("delete from talk where khid = '$id'");
				//删除客户给新感悟点赞
				mysql_query("delete from TalkLaud where khid = '$id'");
				//删除客户给新感悟的回复
				mysql_query("delete from TalkReply where khid = '$id'");
				//删除客户头像
				unlink(ServerRoot.$kehu['ico']);
				//删除客户身份证正面
				unlink(ServerRoot.$kehu['IDCardFront']);
				//删除客户身份证反面
				unlink(ServerRoot.$kehu['IDCardBack']);
				//删除客户手持身份证
				unlink(ServerRoot.$kehu['IDCardHand']);
				//最后删除本客户基本资料
				mysql_query("delete from kehu where khid = '$id'");
				//添加日志
				LogText("客户管理",$Control['adid'],"管理员{$Control['adname']}删除了客户（客户昵称：{$kehu['NickName']}，客户ID：{$kehu['khid']}）");
				$x++;
			}
