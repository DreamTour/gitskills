...
$this_url = $root."pay/";
$alipay_config['partner'] = '2088521114051884';//合作身份者id，以2088开头的16位纯数字
$alipay_config['seller_email'] = 'gzjcwlkj668@126.com';//收款支付宝账号
$alipay_config['key'] = '8yc49c14uexhn9j93m57iievs2qsu3pd';//安全检验码，以数字和字母组成的32位字符
...
/*-----------支付宝接口公用函数,该类是请求、通知返回两个文件所调用的公用函数核心处理文件-------------------------------------------------------------------------------------*/
...
function getHttpResponseGET($url,$cacert_url) {
	...
	curl_setopt($curl, CURLOPT_CAINFO,$cacert_url);//证书地址
	$responseText = curl_exec($curl);
	$error = 'Curl error: ' . curl_error($curl);//如果执行curl过程中出现异常，可打开此开关，以便查看异常内容
	curl_close($curl);
...
class AlipaySubmit {
	...
	var $alipay_config;
	var $alipay_gateway_new = 'https://mapi.alipay.com/gateway.do?';//支付宝网关地址
