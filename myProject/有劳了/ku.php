...
function UserRoot($type){
	if($GLOBALS['KehuFinger'] == 2){
		if($type == "pc"){
			...
			if(strstr($_SERVER['PHP_SELF'],"user.php") !== false  ){
				header("Location:".root."user/usLogin.php"); 
			}else{
				header("Location:".root."seller/seLogin.php"); 
			}
		}elseif($type == "m"){
			$_SESSION['warn'] = "您未登录，请先登录";
			if(strstr($_SERVER['PHP_SELF'],"mUser.php") !== false  ){
				header("Location:".root."m/user/mUsLogin.php"); 
			}else{
				header("Location:".root."mSeller/mSeLogin.php"); 
			}
		}
		exit(0);
	}
...
if($_GET['Delete']=="admin"){
	...
	header("Location:{$root}control/login.php");
	exit(0);
}elseif($_GET['delete']=="client"){
	unset($_SESSION['khid']);
	$_SESSION['warn'] = "您已经退出登录状态";
	if(isMobile()){
...
function option($title,$option,$value){
	...
	if(array_values($option) === $option){
	    foreach($option as $key){
			$key = (string)$key;//解决全等时$key和$value格式不一致的问题
			if($key === $value){//解决$key为0而$value为空的问题
