		...
		$jobList .= "
			<ul class='search-co-list clearfix'>
				<a href='{$root}recruit.php?demandMx_id={$array['id']}' style='display:inline-block'>
					...
					<li class='search-co02'>{$client['ContactName']}</li>
       ...
       <div class="search-condition">
       		<ul class="search-head clearfix">
            	...
            	<li class="search-item"><span>一级分类</span>
                	<?php echo RepeatSelect("classify","type","searchColumn","","请选择劳务主项目",$Classify['type']);?>
                </li>
                <li class="search-item"><span>二级分类</span>
                	<select name="searchColumnChild">
                    	<?php echo IdOption("classify where type = '$SupplyClassify[type]' ","id","name","请选择劳务子项目",$Classify['id']);?>
                    </select>
                </li>
                <li class="search-item" style="margin-right:0"><span>关键词</span>
                	<input name="searchPartKey" type="text" placeholder="请输入搜索关键词" />
                </li>
            </ul>
            <ul class="search-detail">
                ...
                <li class="clearfix hot-place-box">
                    ...
                    <a href="javascript:;" class="search-co-btn fl fz16 col1" id="searchPartButton">点击搜索</a>
                </li>
            </ul>
       </div>
       </form>
...
$(document).ready(function(){
	...
	//搜索表单提交
	$('#searchPartButton').click(function(){
