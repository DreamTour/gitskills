    ...
    if(empty($orderId)){
        ...
        if($bool){
            if($pay['orderType'] == "购物车"){
                ...
                $totalMoney = query("para", "paid = 'totalMoney' "); //消费总金额
                $receiveNumber = query("para", "paid = 'receiveNumber' "); //vip领取次数
                $paySql = mysql_fetch_array(mysql_query("SELECT SUM(money) as t_sum from pay WHERE targetId = '$pay[targetId]' "));//查询消费的总金额
                if ($paySql['t_sum'] >= $totalMoney['paValue']) {
                    mysql_query("UPDATE kehu SET type = 'vip会员', receiveNumber = '$receiveNumber[paValue]' WHERE khid = '$pay[targetId]' ");
                }
            }else{
...
function buycarPay($orderIdGroup){
    ...
    foreach ($orderIdArr as $id) {
        ...
        $buycarSql = query("buycar", "id = '$id' "); //订单管理表
        $paySql = query("pay", "targetId = '$buycarSql[khid]' "); //支付表
        $integralScale = query("para", "paid = 'integralScale' "); //积分比例
        $integral = $paySql['money'] * $integralScale['paValue']; //积分
