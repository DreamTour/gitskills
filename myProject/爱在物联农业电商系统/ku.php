...
function FormSub($data){
    ...
    $data = htmlentities($data, ENT_COMPAT,"utf-8");//字符转换为 HTML 实体。
...
function head($type,$title){
    ...
    if($type == "ad"){
		...
		<link rel='stylesheet' type='text/css' href='".root."control/ku/extend.css'>
		{$jquery}
		...
		<link rel='stylesheet' type='text/css' href='".root."library/m.css'>
		<link rel='stylesheet' type='text/css' href='".root."library/swiper.css'>
		{$jquery}
		<script type='text/javascript' src='".root."library/m.js'></script>
		<script type='text/javascript' src='".root."library/swiper.js'></script>
		";
    }
    if(empty($title)){
...
function Curl($url,$data){
    ...
    curl_setopt($ch, CURLOP_TIMEOUT,6);//设置超时（秒）
...
function wxLogin($ThisUrl){
    ...
    if(empty($_GET['code'])){
        ...
        if(empty($openid)){
            ...
            if(empty($kehu['khid'])){
                $khid = suiji();
                $bool = mysql_query(" insert into kehu (khid,wxOpenid,wxSex,wxNickName,wxAddress,wxIco,wxFollow,updateTime,time)
				values ('$khid','$openid','$sex','$nickname','$address','$ico','$subscribe','$time','$time') ");
                if($bool){
                    $_SESSION['khid'] = $khid;
                    $warn = " insert kehu is ok ";
                }else{
